...
| 普通星图 | 连续天球、离线检索、目标详情、触控浏览与可插拔手势接口 | 否 |
...
普通星图采用球面立体投影和 Canvas 分层绘制，可连续跨越赤经 `0h` 与南北天极。离线包包含
8921 颗 HYG 可见恒星、88 星座中心及连线、M1-M110 精确坐标，以及动态太阳、月球和主要行星；
图层可独立控制星座线、赤道网格、黄道、地平线、深空目标和太阳系目标。当前仓库未内置经过
许可和精度验收的手部关键点模型，因此手势入口会明确显示不可用且不会启动前摄，触控功能不受影响。
